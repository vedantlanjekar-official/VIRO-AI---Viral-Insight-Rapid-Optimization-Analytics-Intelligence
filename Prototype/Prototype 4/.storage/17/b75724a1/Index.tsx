...
import { Shield, TrendingUp, Zap, Database, Activity, DollarSign, Dna, Microscope, FlaskConical, Brain, Sparkles, BarChart3, Mail, Phone, ArrowRight } from 'lucide-react';
          ...
          <div className="relative max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
              {processSteps.map((step, index) => (
                <div key={index} className="relative">
                  ...
                  {/* Arrow connector - hidden on mobile, shown on md and up, except for last item */}
                  {index < processSteps.length - 1 && (
                    <div className="hidden md:flex absolute top-1/2 -right-3 transform -translate-y-1/2 z-10">
                      <ArrowRight className="h-6 w-6 text-[#1E88E5]" strokeWidth={3} />
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="md:hidden flex flex-col items-center mt-4 space-y-2">
              {processSteps.slice(0, -1).map((_, index) => (
                <ArrowRight key={index} className="h-6 w-6 text-[#1E88E5] rotate-90" strokeWidth={3} />
              ))}
            </div>
          </div>
